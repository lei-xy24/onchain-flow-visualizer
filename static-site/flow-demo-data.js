...
export async function loadFlowRecords(chain, { signal, addresses = [] } = {}) {
  ...
  if (requestedAddresses.length && FLOW_API_URL.trim()) {
    return Promise.all(requestedAddresses.map(async (address) => {
      ...
      return validateFlowRecord(await response.json(), chain, address);
    }));
  }
  ...
  const records = await Promise.all(
    files.map(async (file) => {
      ...
      return validateFlowRecord(await response.json(), chain);
    }),
  );
...
export function validateFlowRecord(value, expectedChain, expectedAddress) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error("链上数据必须是对象");
  }
  if (!FLOW_CHAINS[value.chain] || value.chain !== expectedChain) {
    throw new Error("链上数据接口返回了不匹配的网络");
  }
  if (!isEvmAddress(value.address)) throw new Error("链上数据缺少有效的中心地址");
  if (expectedAddress && normalizeAddress(value.address) !== normalizeAddress(expectedAddress)) {
    throw new Error("链上数据接口返回了不匹配的地址");
  }
  if (value.label !== undefined && typeof value.label !== "string") {
    throw new Error("链上数据 label 必须是字符串");
  }
  const seenIds = new Set();
  validateFlowTransfers(value.input, "input", seenIds);
  validateFlowTransfers(value.output, "output", seenIds);
  return value;
}

function validateFlowTransfers(value, field, seenIds) {
  if (!Array.isArray(value)) throw new Error(`链上数据 ${field} 必须是数组`);
  value.forEach((transfer, index) => {
    const path = `${field}[${index}]`;
    if (!transfer || typeof transfer !== "object" || Array.isArray(transfer)) {
      throw new Error(`${path} 必须是对象`);
    }
    if (typeof transfer.id !== "string" || !transfer.id.trim()) throw new Error(`${path}.id 无效`);
    if (seenIds.has(transfer.id)) throw new Error(`链上数据包含重复转账 id：${transfer.id}`);
    seenIds.add(transfer.id);
    if (!isEvmAddress(transfer.address)) throw new Error(`${path}.address 无效`);
    if (typeof transfer.time !== "string" || !Number.isFinite(Date.parse(transfer.time))) throw new Error(`${path}.time 无效`);
    if (typeof transfer.rawAmount !== "string" || !RAW_AMOUNT_PATTERN.test(transfer.rawAmount)) throw new Error(`${path}.rawAmount 无效`);
    if (!Number.isInteger(transfer.decimals) || transfer.decimals < 0 || transfer.decimals > 255) throw new Error(`${path}.decimals 无效`);
    if (typeof transfer.asset !== "string" || !transfer.asset.trim()) throw new Error(`${path}.asset 无效`);
    if (transfer.assetAddress !== null && !isEvmAddress(transfer.assetAddress)) throw new Error(`${path}.assetAddress 无效`);
    if (typeof transfer.txHash !== "string" || !/^0x[0-9a-fA-F]{64}$/.test(transfer.txHash)) throw new Error(`${path}.txHash 无效`);
    if (transfer.tag !== undefined && typeof transfer.tag !== "string") throw new Error(`${path}.tag 无效`);
  });
}
