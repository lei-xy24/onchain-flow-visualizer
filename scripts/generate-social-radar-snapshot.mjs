...
function buildInstructions(config) {
  return [
    ...
    "topicType 是稳定、唯一的英文 snake_case 主题标识。dataMode=market 时，topicType 必须逐字等于 allowedMarketTopicTypes 中的一个值，禁止创建 bitcoin_supply、ethereum_scaling 等自定义市场标识。",
    "只有主题本身直接讨论对应区块链市场时才能使用 dataMode=market；找不到逐字匹配且直接相关的市场模板时，必须创建自己的 topicType 并使用 dataMode=evidence。",
